...
# based on video: DCGAN implementation from scratch
...
for epoch in range(num_epochs):
    ...
    x_fake = gen(noise)
    disc_real = disc(x_real).view(-1) # what discirminator outputs on the real ones
    lossD_real = criterion(disc_real, torch.ones_like(disc_real))
    disc_fake = disc(x_fake).view(-1) # D(G(z)
    ...
    # Train the Generator: minimize log(1 - D(G(z))) --> max log(D(G(z)))
    output = disc(x_fake).view(-1)
